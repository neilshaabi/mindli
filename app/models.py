...
class User(UserMixin, db.Model):
    ...
    gender: so.Mapped["Gender"] = so.mapped_column(sa.Enum(Gender))
...
class Therapist(db.Model):
    ...
    certifications: so.Mapped[Optional[str]] = so.mapped_column(sa.Text)
...
class SessionType(db.Model):
    ...
    session_duration: so.Mapped[int] = so.mapped_column(sa.Integer)  # Duration in minutes
